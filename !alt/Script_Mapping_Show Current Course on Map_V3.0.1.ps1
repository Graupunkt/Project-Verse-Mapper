...
$img = [system.drawing.image]::FromFile("$script:ScriptDir\maps\systems\$CurrentSystem.jpg")
...
$pictureBox2.Location = New-Object System.Drawing.Size(0,1)
$img2 = [system.drawing.image]::FromFile("$script:ScriptDir\maps\planets\$CurrentPlanet.jpg")
$pictureBox2.Size = New-Object System.Drawing.Size($img2.Width,$img2.Height)
$pictureBox2.Image = $img2



$SystemCanvas.Controls.SetChildIndex($pictureBox, 2)
$SystemCanvas.Controls.SetChildIndex($pictureBox2, 1)
$SystemCanvas.controls.add($pictureBox)
$SystemCanvas.controls.add($pictureBox2)
#>
...
$formGraphics.DrawLine($pen, 1, 1, 190, 190)
$formGraphics.FillEllipse($myBrush, 5, 5, 180, 180)
...
$rect = new-object Drawing.Rectangle 0, 0, 190, 190
$xxx = $SystemCanvas.createGraphics()

$SystemCanvas.add_paint({
    $xxx.FillEllipse($myBrush, $rect) # draw an ellipse using rectangle object
    $mypen.color = "red" # Set the pen color
    $mypen.width = 5     # ste the pen line width
    $xxx.DrawLine($mypen, 10, 10, 190, 190) # draw a line
    $xxx.DrawLine($mypen, 190, 10, 10, 190) # draw a line
})

#$TabSystem.Controls.SetChildIndex($SystemGraphics, 1)
...
$starttick={
    ...
    #write-host $script:i
    $Progress.Text = "Count: $script:i"
    if($script:i -gt 10){$SystemLineSettings.Color = "black"}
    if($script:i -gt 20){$SystemLineSettings.Color = "red"}
    if($script:i -gt 30){$SystemLineSettings.Color = "green"}
    #$TabSystem.add_paint({$SystemGraphics.DrawLine($SystemLineSettings, 1, 1, 800, 800)})
    #$form.refresh()
}
