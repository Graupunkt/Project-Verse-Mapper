...
$MapXpixels = $SystemImage.Width - $MarginLeft - $MarginRight
$MapYpixels = $SystemImage.Height - $MarginTop - $marginBottom
...
$sizedot = 7
$PosSettings = new-object Drawing.SolidBrush ("green")
$Possizedot = 15
$Lastobject = $false
...
### UPDATE LOOP FOR SYSTEM
    ...
    foreach ($dataset in $PlanetData){
        ...
        $Prev_xValue = $MapCentreX + ([double]$PlanetData[$previous].Longitude * $LongPix)
        $Prev_yValue = $MapCentreY - ([double]$PlanetData[$previous].Latitude  * $LatPix)

        #draws red lines between the black dots
        if($Global:count  -gt 0){$TabSystem.add_paint({$SystemGraphics.DrawLine($LineSettings, $Prev_xValue, $Prev_yValue, $xValue, $yValue)}.GetNewClosure())}

        #black dots at each update
        $TabSystem.add_paint({$SystemGraphics.FillEllipse($DotSettings, $xValue-($sizedot/2), $yValue-($sizedot/2), $sizedot, $sizedot )}.GetNewClosure())
...
### UPDATE LOOP FOR PLANET
    ...
    foreach ($dataset in $PlanetData){
        ...
        #draws red lines between the black dots
        if($Global:count  -gt 0){$TabPlanet.add_paint({$SystemGraphics.DrawLine($LineSettings, $Prev_xValue, $Prev_yValue, $xValue, $yValue)}.GetNewClosure())}

        #black dots at each update
        $TabPlanet.add_paint({$SystemGraphics.FillEllipse($DotSettings, $xValue-($sizedot/2), $yValue-($sizedot/2), $sizedot, $sizedot )}.GetNewClosure())

        #Shows a big green dot at the current/last position
        if(($Global:count + 1) -eq $PlanetData.Count){
            #MAKRS THE CURRENT POSITION
            $TabPlanet.add_paint({$SystemGraphics.FillEllipse($PosSettings, $xValue-($Possizedot/2), $yValue-($Possizedot/2), $Possizedot, $Possizedot )})
        }
        $Global:count ++ 
    }
####################################################################################################
